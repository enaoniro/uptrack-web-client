...
const CantonContextProvider = (props) => {
  ...
  const getCantons = async () => {
    const response = await fetch("https://uptrack.onrender.com/cantons");
  ...
  const addCanton = async (pCanton) => {
    ...
    try {
      await fetch("https://uptrack.onrender.com/cantons", {
        method: "POST",
        body: JSON.stringify(pCanton),
        headers: { "Content-Type": "application/json" },
      });
  ...
  const deleteCanton = async (pCantonId) => {
    ...
    try {
      await fetch("https://uptrack.onrender.com/cantons/" + pCantonId, {
        method: "DELETE",
      });
