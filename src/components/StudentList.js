...
const StudentList = ({ showDetails, setShowDetails, grup }) => {
  console.log(grup);
  console.log(grup.email);

  console.log("student list is rendered");

  const navigate = useNavigate();

  const { studentsInGrup, setStudentsInGrup, getStudentsInGrup, studentList } =
  ...
  if (!studentList || !studentList.length) {
    ...
    return (
      <div className="container-fluid bg-white d-flex align-items-center justify-content-center">
        <Container fluid myclass className="mt-10 p-3 bg-white">
          <Table responsive bordered hover>
            <thead className="myclass">
